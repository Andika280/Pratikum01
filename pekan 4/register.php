...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($username) && !empty($password) && !empty($email)) {
        $sql = "INSERT INTO users (username, email, whatsapp, password) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssss", $username, $email, $whatsapp, $password); 
        if ($stmt->execute()) {
            $pesan = "<p style='color:green;'>Registrasi sukses! Silakan Login</p>";
        } else {
            $pesan = "<p style='color:red;'>Gagal mendaftar: " . $stmt->error . "</p>";
        }
        $stmt->close();
    }
}
